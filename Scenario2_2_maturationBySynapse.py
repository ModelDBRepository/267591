...
class PurkinjeCell(SynFront):
    ...
    def select_winners(self,constellation,somaID):
        ...
        # caluculate the number of synapses in a whole neuron
        neuron = self.get_neuron(constellation)
        wsyn_len = neuron.num_synapses
        
        roots = self.get_children(constellation)

        if self.stage == 1:
            if wsyn_len > WholeCheck_syn_1:
                if (verbose >= 1) or (verbose_cell == somaID):
                    print(nname,"reached whole synapse threshold =",WholeCheck_syn_1,"at",constellation.cycle)
                ...
                for root in roots:
                    ...
                    branch_syn = root.get_branch_synapses(constellation)
                    esyn_len = len(branch_syn) # get the number of synapses for each branch
            
                    if esyn_len < first_syn_th:
                        if (verbose >= 1) or (verbose_cell == somaID):
                            print(root_name,"at 1st selection: num front =",esyn_len, ", retract")
                        self.retract_branch(constellation,root)
                    else:
                         if (verbose >= 1) or (verbose_cell == somaID):
                            print (root_name,", 1st threshold, EXTEND further with____",esyn_len,"fronts")

        elif self.stage == 2: #  final selection
            if wsyn_len > WholeCheck_syn_2:
                if (verbose >= 1) or (verbose_cell == somaID):
                    print(nname,"reached whole fronts threshold 2 =",WholeCheck_syn_2,"at",constellation.cycle)
                
                self.stage = 3
                
                total_syn = [0.] * self.num_children
                # generate total_front list of each dendrite
                nd = 0
                for root in roots:
                    branch_syn = root.get_branch_synapses(constellation)
                    esyn_len = len(branch_syn)
                    total_syn[nd] = esyn_len
                    nd += 1

                nd = 0
                for root in roots:
                    root_name = root.get_branch_name()
                    if total_syn[nd] == max(total_syn):
                        if (verbose >= 1) or (verbose_cell == somaID):
                            print (root_name,", 2nd threshold, EXTEND further with____",total_syn[nd],"fronts")
                    else:
                        if (verbose >= 1) or (verbose_cell == somaID):
                            print(root_name,"at 2nd selection: num front =",total_syn[nd], ", retract")
                        self.retract_branch(constellation,root)
...
if __name__ == '__main__':
    ...
    fname = "db_files_inDeigo/Note46D_S2_2B_90n30_" + str(s) + ".db"
