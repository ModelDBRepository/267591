...
Whole_signal_1st = 500
Whole_signal_2nd = 1200
...
class PurkinjeCell(SynFront):
    ...
    def manage_front(self,constellation):
        ...
        if self.swc_type == 1:
            ...
            if self.has_synapse(): # front has a synapse and integrates syn_input each cycle
                ...
                soma = constellation.front_by_id(somaID)
                if (soma.is_status2()) and (not self.is_growing()):# 2nd retractions done and not grwoing tip
    ...
    def select_winners(self,constellation,somaID):
        ...
        # setups for signal detection
        total_input = [0.] * self.num_children # store summed signal from each synapses in a dendrite
        ...
        for root in roots:
            syns = root.get_branch_synapses(constellation)
        ...
        if self.stage == 1:
            if sum_tot > Whole_signal_1st:
                if (verbose >= 1) or (verbose_cell == somaID):
                    print(nname,"reached whole signal threshold =",Whole_signal_1st,"at",constellation.cycle)
                ...
                if (verbose >= 1) or (verbose_cell == somaID):
                    ...
                    print (nname,"has",neuron.num_fronts,"fronts with",neuron.num_synapses,"synapses, mean summed input per dendrite: {:5.2f}".format(mean_tot))
    

                # retract by by synaptic input
                nd = 0
                for root in roots:
                    root_name = root.get_branch_name()
                    if total_input[nd] < mean_tot:
                        if (verbose >= 1) or (verbose_cell == somaID):
                            print (root_name," summed input of {:5.2f}".format(total_input[nd]),"and is retracted")
                            self.retract_branch(constellation,root)
                    else:
                        if (verbose >= 1) or (verbose_cell == somaID):
                            print (root_name," summed input of {:5.2f}".format(total_input[nd]),"and stays")
                    nd += 1
    
    
    
        elif self.stage == 2: #  final selection
            if sum_tot > Whole_signal_2nd:
                if (verbose >= 1) or (verbose_cell == somaID):
                    print(nname,"reached whole signal threshold 2 =",Whole_signal_2nd,"at",constellation.cycle)
                
                self.stage = 3
                self.set_status2() # flag as done with second selection

                nd = 0
                for root in roots:
                    root_name = root.get_branch_name()
                    if total_input[nd] == max(total_input):
                            if (verbose >= 1) or (verbose_cell == somaID):
                                print (root_name,", 2nd selection, won with____",total_input[nd],"total signal input")
                    else:
                        if (verbose >= 1) or (verbose_cell == somaID):
                            print(root_name,"at 2nd selection: total signal input =",total_input[nd], ", retract")
                        self.retract_branch(constellation,root)
                        
                    nd += 1
    
                soma = constellation.front_by_id(somaID)
                soma.set_status2() # flag as done with second selection
...
if __name__ == '__main__':
    ...
    #iname = "db_files/Note45_part1_S1test_c16_seed1_115mins.db"
    fname = "db_files_inDeigo/Note46D_S3_500n1200_" + str(s) + ".db"
    ...
    admin = Admin_agent(63,fname,sim_volume,neuron_types,verbose=1,max_neurons=14000,\
                        max_fronts=2000000,max_arcs=100,max_active=240000,max_substrate=1280,\
